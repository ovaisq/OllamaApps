...
def main():
    ...
    ollama_host = os.getenv('OLLAMA_API_URL', 'http://127.0.0.1')
    ...
    combined_review = f"## Code Review for PR #{args.pr_number} (using {args.model})\n\n" + "\n".join(reviews)
    ...
    if len(combined_review) > max_length:
        ...
        for i, chunk in enumerate(chunks):
            try:
                ...
                pr.create_issue_comment(f"Review Part {i+1}:\n\n{chunk}")
            except Exception as e:
