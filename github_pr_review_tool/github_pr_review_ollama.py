...
def format_review_response(json_response):
    ...
    if overall_comments:
        ...
        formatted_comments = "_No overall comments provided._"
...
def main():
    ...
    ollama_host = os.getenv('OLLAMA_API_URL', 'http://192.168.3.16')  # Default URL if not provided
    ollama_model = os.getenv('OLLAMA_MODEL', 'gpt-oss:120b')  # Default model if not provided
    ...
    # Initialize GitHub client with the updated method
    g = Github(auth=Auth.Token(github_token))  # Updated constructor
    ...
    for file in files:
        ...
        review_response = get_ollama_review(diff_text, file.filename, ollama_model)
    ...
    combined_review = (
        ...
        + "\n---\n\n".join(reviews) +
        "\n\n---\n\n"
        f"_This review was generated by an automated code review bot using `{ollama_model}`_"
    )
